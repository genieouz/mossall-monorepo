...
let Activitieservice = Activitieservice_1 = class Activitieservice extends abstract_service_1.AbstractService {
    ...
    async CancelByAdminDemande(payload) {
        ...
        const { user, initialValue } = payload;
        const activity = await this.createDemandeActivity(user, initialValue);
        const message = `l'admin ${user.firstName} ${user.lastName}(${user.email}) a annulé la demande N°${(_a = activity.initialValue) === null || _a === void 0 ? void 0 : _a.number}. 
    ${activity.message}`;
        const org = await this.organizationService.findOneById(user.organization);
        this.logger.log(`Organization ${org.name}: ${message}`);
        await this.insertOne(Object.assign(Object.assign({}, activity), { message, scope: activity_scope_enum_1.ActivityScope.demande }));
    ...
    async RejectDemande(payload) {
        ...
        const { user, initialValue } = payload;
        const activity = await this.createDemandeActivity(user, initialValue);
        const message = `l'admin ${user.firstName} ${user.lastName}(${user.email}) a rejeté la demande N°${(_a = activity.initialValue) === null || _a === void 0 ? void 0 : _a.number}. 
    ${activity.message}, 
    ...
    async ValidateDemande(payload) {
        ...
        const { user, initialValue } = payload;
        const activity = await this.createDemandeActivity(user, initialValue);
        const payment = await this.paymentService.findOne({ 'meta.demandeId': String(((_a = activity.initialValue) === null || _a === void 0 ? void 0 : _a.id) || ((_b = activity.initialValue) === null || _b === void 0 ? void 0 : _b._id)) });
        const message = `l'admin ${user.firstName} ${user.lastName}(${user.email}) a validé la demande N°${(_c = activity.initialValue) === null || _c === void 0 ? void 0 : _c.number}. 
    ${activity.message}, 
    ...
    async PayeDemande(payload) {
        ...
        const { user, initialValue } = payload;
        const activity = await this.createDemandeActivity(user, initialValue);
        const message = `l'admin ${user.firstName} ${user.lastName}(${user.email}) a marqué "Remboursé" la demande N°${(_a = activity.initialValue) === null || _a === void 0 ? void 0 : _a.number}. 
    ${activity.message}`;
        const org = await this.organizationService.findOneById(user.organization);
        this.logger.log(`Organization ${org.name}: ${message}`);
        await this.insertOne(Object.assign(Object.assign({}, activity), { message, scope: activity_scope_enum_1.ActivityScope.demande }));
