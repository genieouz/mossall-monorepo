...
import { ALALApiResponseData, Disburse } from '../schemas/interfaces/alal-customer.interface';
...
export class PaymentController {
    ...
    async updateDisburse(
        ...
        const hash = crypto.createHmac('sha512', ALAL_API_KEY).update(JSON.stringify(body)).digest('hex');
        ...
        if (hash == req.headers['x-alal-signature']) {
            ...
            const found = await this.paymentService.findOne({ reference: disburse.reference });
            ...
            if(!found) {
                ...
                const demandeByRef = await this.demandeService.findOne({ transactionReference: disburse.reference });
                ...
                const date = new Date();
                const demande = await this.demandeService.findOneAndUpdate({ _id: demandeByRef.id}, 
