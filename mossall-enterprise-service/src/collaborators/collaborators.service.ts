...
export class CollaboratorsService {
  ...
  async inviteCollaborator(
    payload: InviteCollaboratorInput,
    categorySocioProId: string,
    currentUserId: string,
  ) {
    // find categorySocioPro or throw error
    ...
    if (found) {
      throw new ConflictException(
        'Email ou Téléphone ou Identifiant ou compte bancaire déjà utilisé',
      );
    }
    const tmpUser = await this.userSerive.insertOne({
      ...payload,
      id: new ObjectId(),
      organization: currentUser.organization,
      role: 'COLLABORATOR',
      password: this.authService.hashPassword(password),
      categorySocioPro: categorySocioProId,
    } as any);
  ...
  async updateCollaborator(
    ...
    return this.userSerive.updateOneById(collaboratorId, {...payload, categorySocioPro: categorySocioProId});
  }
}
